...
Purpose: Documents all 32 operational processes with triggers, happy paths, exception paths, escalation paths, and outcomes to establish how Happy Feet's work actually flows end-to-end.
...
Dependencies: DOMAIN_OVERVIEW.md, BUSINESS_CAPABILITY_MODEL.md, ORGANIZATION_MODEL.md
...
**Source:** PRODUCT_OVERVIEW_v2.md, DOMAIN_OVERVIEW.md, BUSINESS_CAPABILITY_MODEL.md, ORGANIZATION_MODEL.md
**Date:** 2026-06-04
**Status:** Complete
...
**Outcome:** All downstream modules (admissions, attendance, fee management, progress tracking) have the operational framework they need. No admissions, attendance marking, or invoice generation can begin until all 9 steps are confirmed complete.
...
2. Branch Admin assigns programme and class. A student profile record is initiated in the student information system at this point. Status: **Approved**.
...
### P07 — QR Check-In and Check-Out at Gate

**Trigger:** Each school day, as students arrive and depart.
**Actors:** Coordinator (primary gate role — QR scanning), Teacher (backup), Branch Admin (oversight).

**Happy path — Check-In:**
1. Parent or authorised person arrives at gate with the child; presents the student QR code from the parent portal.
2. Coordinator opens the gate scanning screen on the tablet or phone.
3. Coordinator scans the student QR code.
4. System displays: student photo, name, class, and any active allergy alerts.
5. Coordinator confirms → CHECK_IN event logged with timestamp; attendance status set to **Present**.

**Happy path — Check-Out:**
1. Authorised person arrives at the gate to collect the child.
2. Coordinator scans the student QR code → system displays the student's active authorised pickup list.
3. Coordinator selects which listed person is collecting.
4. Coordinator confirms → CHECK_OUT event logged with timestamp and pickup person ID.
5. For Day Care students: system automatically computes hours in care (CHECK_OUT − CHECK_IN). Full day (≥4 hours) = 1.0 billing unit; half day (<4 hours) = 0.5 billing unit.

**Exception paths:**
- *QR system unavailable:* Teacher marks attendance manually from the class attendance screen. Manual mark logged as "Manual mark — QR system unavailable." Day Care billing for manual marks requires Branch Admin to retrospectively record checkout.
- *Student QR not available at gate:* Coordinator searches for the student by name and marks manually with a logged note.
- *CHECK_OUT not recorded at end of day:* Branch Admin is alerted if any students have a CHECK_IN with no corresponding CHECK_OUT. Branch Admin can manually record a checkout retrospectively with a logged reason. Day Care billing defaults to end of configurable school day for unscanned checkouts — this is a fallback, not an accurate measurement.

**Escalation path:**
- *Person presenting at gate is NOT on the authorised pickup list:* System blocks the checkout and displays: "This person is not on the authorised pickup list. Contact Branch Admin before releasing the child." Coordinator contacts Branch Admin immediately. Branch Admin decides whether to proceed (with a governance log entry) or refuse release.

**Outcome:** Arrival and departure are confirmed against a known student record and verified pickup list. Day Care billing units are derived from gate timestamps, not manual estimates.
...
**Actors:** Coordinator (any class, primary gate role after QR), Teacher (own class — primary marker), Branch Admin (oversight, corrections), Admin (unlimited corrections).

**Happy path:**
1. Attendance is derived from QR CHECK_IN events (see P07). Students with a CHECK_IN confirmed as Present.
2. Students without a CHECK_IN by the end of the marking window appear as pending.
3. Teacher reviews pending students and bulk-confirms as Absent or adjusts individually.
4. Absent mark → immediate in-app notification fires to the parent.
5. Attendance record locked for the day.

**Exception paths:**
- *Teacher does not mark within the window:* Branch Admin is alerted by an in-app notification. Branch Admin can bulk-mark the entire class as Present with a logged reason.
- *Attendance on a school holiday:* Blocked for all roles. No override exists.
- *Manual marking day (QR unavailable):* Teacher marks each student Present, Absent, Late, or Half-Day directly on the attendance screen. Logged as manual mark.
- *Late arrival:* CHECK_IN received after the configurable late threshold → attendance status set to Late.
- *Half-day:* Teacher marks manually as Half-Day.
...
**Happy path:**
1. Teacher opens the attendance screen for their class. The daily activity log entry panel appears on the same screen — no separate navigation required.
...
- *Entry made in error:* Daily activity log entries are immutable after saving. Corrections are appended as new entries referencing the original. The original entry remains.
...
**Happy path:**
1. Billing units have accumulated throughout the period from QR check-in/check-out timestamps (see P07). Full day (≥4 hours) = 1.0 unit; half day (<4 hours) = 0.5 unit.
2. Accountant runs the Day Care billing summary export from M7.
3. Export shows per-student billing units for the period.
4. Accountant takes the export to CoFee to generate the Day Care invoices for that period.

**Exception paths:**
- *Unscanned checkouts:* Students with a CHECK_IN but no CHECK_OUT default to the end of the configurable school day for billing purposes. Branch Admin is alerted at end of day and can retrospectively record the checkout (see P07). Uncorrected defaults are included in the billing summary as fallback values.
- *Manual attendance days:* Days where QR was unavailable and attendance was manually marked may not have accurate hours-in-care data. Branch Admin flags these for Accountant review.
...
**Happy path (no outstanding dues): [IRREVERSIBLE]**
...
**Happy path:**
1. Teacher observes a concern during the school day. Teacher opens the attendance or daily activity log screen for the student.
...
**Happy path — Deactivating an entry:**
1. Branch Admin navigates to the student profile → Authorised Pickup List.
...
**Escalation path:**
- *Unregistered person presents at gate during checkout:* Escalates to Branch Admin immediately (see P07 exception path). No system escalation — Coordinator physically contacts Branch Admin.
...
6. Branch Admin creates all student profiles and enters opening balances.
...
2. Actor selects the export type (student list, enrollment, attendance, staff attendance, invoice list, payment list, communication log, governance report, expense log, daily activity log, welfare concern report, leave register).
...
2. Digest includes:
   - Override actions (attendance corrections beyond 30 days, Bonafide/TC with outstanding dues, capacity overrides, document waivers, Migration Mode events)
...
| Student arrives at school gate | P07 — QR Check-In |
| Student departs from school gate | P07 — QR Check-Out |
...
Capacity overrides, document waivers, Bonafide with outstanding dues, corrections beyond 30 days — all permitted, but each requires higher authority, a recorded reason, and a governance log entry.
