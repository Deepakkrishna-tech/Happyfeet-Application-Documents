...
Dependencies: DOMAIN_OVERVIEW.md, BUSINESS_CAPABILITY_MAP.md, ORGANIZATION_MODEL.md, BUSINESS_PROCESSES.md, BUSINESS_RULES.md
...
| Coordinator | Human | Own branch — intake, gate, inbox | Advance admissions, verify authorised pickup at gate, mark student attendance for any class, manage school inbox |
...
The daily record of a student's attendance status for academic purposes. Teacher-managed. Independent from Day Care billing records.

**Key attributes:** Student, date, class, status (Present / Absent / Late / Half-Day), marked by (staff actor and role), correction log

> Note: CHECK_IN and CHECK_OUT timestamps for Day Care billing are maintained as separate operational records (see Day Care Billing Plan — §2.35 and P07B), not on the Attendance Record.

**Status derivation:**
- Teacher (or authorised secondary actor) marks Present, Absent, Late, or Half-Day directly on the class attendance screen
- If no record is created within the marking window: reminder notification fires to the responsible Teacher; if still unmarked, escalation notification fires to Branch Admin; no attendance record exists for the student until created by authorised staff or bulk-marked by Branch Admin

**Valid statuses:** Present / Absent / Late / Half-Day

**Day Care:** Day Care billing is independent from attendance status and is governed by the assigned Day Care Billing Plan (§2.35). Attendance records and Day Care billing records are separate business concepts.
**Relationships:** Belongs to Student and Class; triggers parent notification #3 on Absent; feeds Progress Card attendance snapshot at publication
...
**Relationships:** Belongs to Student; verified at gate by Coordinator using the student profile (system blocks release if person not listed — see P07A); readable by Teacher (own class) and Coordinator (gate verification); every change triggers mandatory parent notification (bypasses opt-out)
...
A distance band with a fixed monthly billing rate for school-owned transport.
...
### 2.35 Day Care Billing Plan

The billing configuration assigned to a Day Care programme or student. Determines how billing is calculated for each billing period. All Day Care billing behaviour is governed by the assigned plan.

**Key attributes:** Billing mode (Monthly \| Weekly \| Day-wise \| Hour-wise \| Term-wise), hour plan (for Hour-wise mode: 2 \| 3 \| 4 \| 5 \| 6 \| 7 \| 8 \| 9 \| 10 hours), rate/amount, scope (programme-level or student-level), effective dates

**Billing mode notes:**
- Monthly, Weekly, Term-wise: flat-rate fees — billing is not duration-derived
- Day-wise: billed per attendance day
- Hour-wise: billed per configured hour plan; staff-recorded check-in/check-out timestamps support calculations where the plan requires them

**Invariant:** Every Day Care student must have an assigned Day Care Billing Plan before billing can be computed for that student.

**Managed by:** Branch Admin (configured in M7 fee settings)

**Relationships:** Assigned to Day Care programme or student; referenced by Day Care Billing Summary export (see §4 Documents); governs EX-32 and FR-03 billing behaviour; check-in/check-out timestamps (see CheckInOutEvent) support billing where required by the plan

**SupplementaryService type:** `DAY_CARE` — sits alongside `TRANSPORT` on the student enrollment record as an optional supplementary service (DECISION-008). The enrollment record stays singular. Billing generates one consolidated invoice with multiple line items, one per active supplementary service. The `DAY_CARE` line item is governed by the assigned Day Care Billing Plan; the `TRANSPORT` line item is governed by the Transport Zone rate. SupplementaryServiceType enum values: `TRANSPORT`, `DAY_CARE`.
...
| Event | Trigger | Primary Effect |
|---|---|---|
| Attendance Marked by Teacher | Teacher marks student Present / Absent / Late / Half-Day on class attendance screen | Attendance record created; parent notification #3 fires immediately on Absent (opt-outable) |
| Attendance Not Marked — Reminder | Marking window closes with no attendance record for one or more students | Reminder notification fires to responsible Teacher |
| Attendance Not Marked — Escalation | Reminder sent; no record created after reminder | Escalation notification fires to Branch Admin |
| Day Care Check-In Recorded | Authorised staff records Day Care student arrival time | CHECK_IN timestamp stored as operational record (see P07B) |
| Day Care Check-Out Recorded | Authorised staff records Day Care student departure time at departure | CHECK_OUT timestamp stored as operational record; billing applied per assigned Day Care Billing Plan where required |
| Unregistered Person at Gate | Person not on pickup list presents at departure (see P07A) | System blocks; Coordinator escalates to Branch Admin; Branch Admin proceeds with governance log entry or refuses release |
...
### 3E — Progress & Assessments Events
...
| Day Care Billing Summary Export | Accountant (generates from M7) | Computed billing amounts (per assigned Day Care Billing Plans and operational timestamp records) exported for CoFee invoice generation | N/A — export only |
...
| Attendance Record | Student Profile / Class / Date | Teacher-entered status (Present / Absent / Late / Half-Day); correction log |
