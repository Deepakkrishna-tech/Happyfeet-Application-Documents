...
Four rules govern this data category without exception:
...
- **Fee management** — the canonical financial ledger (single source of payment truth); fee structures, idempotent reconciled CoFee import, lightweight invoice generation with derived states (Draft, Issued, Partially Paid, Paid, Overdue, Void, Credited), Razorpay payment links, Day Care billing plans, and a parent-facing self-serve fee status with one-tap payment
...
This rule prevents the class of errors where one module's dates drift out of sync with another's — a common failure point in systems where each module manages its own calendar independently.
