...
**EX-12a:** Student records are branch-scoped. A student enrolled at Branch A cannot be accessed from Branch B by any role below Admin. A student transferring between branches requires discontinuation at the source branch (per EX-49) and new enrollment at the destination branch (per the M5 enrollment workflow). No cross-branch profile transfer mechanism exists. The student's history at the source branch is permanently retained in that branch's records.
...
**EX-21a:** Admission ID assignment follows a dual-path model. For students migrated from existing school records (pre-go-live): Admin or Branch Admin manually enters the historically assigned Admission ID during Migration Mode; the system preserves it as the permanent ID. For new admissions after go-live: the system auto-generates the Admission ID at profile creation. In both cases the ID is permanent (see INV-05). The system enforces uniqueness at save time. Format consistency of migrated IDs is a data-preparation responsibility before entry. (See DECISION-009.)
...
**EX-32:** Day Care billing is governed by the Day Care Billing Plan assigned to the student or programme. Supported billing modes: Monthly (fixed monthly fee), Weekly (fixed weekly fee), Day-wise (fee per attendance day), Hour-wise (fee per configured hour plan; plans: 2–10 hours), Term-wise (fixed term fee). Staff-recorded check-in/check-out timestamps support operational tracking and billing calculations where required by the assigned plan. Billing for flat-rate modes (Monthly, Weekly, Term-wise) is not duration-derived. Day Care billing records and student attendance records are independent — a Day Care student's billing is not derived from their attendance status.

**EX-33:** Attendance is marked directly by Teacher (or an authorised secondary actor: Assigned Secondary Teacher, Coordinator, or Branch Admin) on the class attendance screen. There is no prerequisite infrastructure for attendance marking. If no attendance record is created within the marking window, a reminder notification fires to the responsible Teacher. If no record is created after the reminder, an escalation notification fires to Branch Admin. No attendance record exists for a student on a given date until Teacher or authorised staff creates one, or Branch Admin bulk-marks the class. Valid attendance statuses: Present, Absent, Late, Half-Day.
...
**EX-35:** The 9-step academic year setup must complete in prescribed sequence before any dependent module (admissions, attendance, invoice generation, progress & assessments) can operate.
...
**EX-46:** All programmes except Event-type participate in progress & assessments.
...
**IM-03:** For billing modes where the Day Care Billing Plan requires timestamp data (such as Hour-wise), billing accuracy depends on authorised staff recording check-in and check-out timestamps correctly for each Day Care student each day. Days where required timestamps are not recorded produce incomplete operational records. Branch Admin is alerted at end of day when check-out is not recorded for a Day Care student (see P07B). Plan modes that do not derive fees from duration (Monthly, Weekly, Term-wise) are unaffected by missing timestamp data.
*Why implied:* Timestamp-dependent plans cannot compute accurate billing without both check-in and check-out records; flat-rate plans do not rely on timestamp data for fee calculation.
...
**FR-03 — Day Care billing is governed by the assigned Day Care Billing Plan.**
Billing behaviour is determined by the plan mode (Monthly, Weekly, Day-wise, Hour-wise, Term-wise). Staff-recorded check-in/check-out timestamps support operational tracking and billing calculations where required by the plan (see EX-32). When check-out is not recorded by end of day, Branch Admin is alerted and authorised staff may record it retrospectively (see P07B). Unresolved missing timestamps are flagged in the Day Care billing summary export as incomplete records.
