<!-- ABOUT
File: BUSINESS_CAPABILITY_MODEL.md
...
|---|---|---|
| Admissions Processing | Accept applications via public form or walk-in; assign programme and class; verify documents against programme requirements; waive individually with a recorded reason | School has a searchable, auditable record of every child who has ever applied |
...
The school must know who is on premises each day, track arrivals and departures at the gate, and bill Day Care children accurately by hours in care.
...
| Day Care Billing Unit Computation | Hours in care = CHECK_OUT − CHECK_IN; full day (≥4 hours) = 1.0 unit; half day (<4 hours) = 0.5 unit | Day Care billing is derived from actual gate timestamps, not from assumptions |
...
| Day Care Billing Summary Export | Computed from QR billing units; Accountant exports and takes to CoFee for invoice generation | Day Care billing is derived from the system's attendance record, not from manual tallying |
...
| Gate Pickup Verification | Coordinator scans student QR at checkout; system displays active pickup list; Coordinator selects who is collecting; system blocks and alerts if person is not listed | No child can be released to an unregistered person — the block is a system enforcement, not a policy reminder |
...
| Override Accountability | Every exception path — capacity override, document waiver, attendance correction beyond 7 days, Bonafide with outstanding dues — requires a recorded reason and generates a governance log entry | Exceptions are sanctioned, not bypassed; every deviation from normal operation is documented |
...
| Migration Checklist | Six-item checklist (student profiles, pickup lists, opening balances, CoFee import, academic year setup, staff accounts); Admin cannot exit Migration Mode until all items are green or individually acknowledged with a logged reason | Go-live requires a deliberate, documented confirmation that the school has prepared its data — not just an Admin clicking a button |
